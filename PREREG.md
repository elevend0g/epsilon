...
| Q2 prediction from §4.2 finding 12 (Regime comparison) | **Preregistered before Phase 3 runs, from the clamped-arm margin asymmetry (§4.2, `RESULTS.md` finding 12): Regime 2 is predicted to show higher cross-condition transfer separation than Regime 1 on the §5.4 coverage probe (arm A vs. B2 at P2).** §5.4's existing Regime-1 falsification threshold (`AUROC < 0.65`) is unchanged; this is an additional directional prediction, not a new numeric gate. A result where Regime 2 does *not* separate more than Regime 1 is a disconfirmation to report, not to explain away | FIXED (prediction, stated before Phase 3 data exists); **outcome PENDING Phase 3** |
