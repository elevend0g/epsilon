...
| Total real-seed commitment (§2.6) | 2 supervision regimes × 3 seeds = **6 runs**, budget is per-run: `6 × 102,400 = 614,400` optimizer steps total, before any Phase 2-4 instrumentation | **PROVISIONAL on §3.1.7** — `102,400` came from full-rank pilots; the six real runs train at the bottleneck. If the rank-36 throwaway pilot's `S*` exceeds 25,600, budget re-derives as `4 × max(25600, rank-pilot S*)` and this total moves up accordingly, before any of the six launch. Confirm affordability against the final number, not this one |
...
| Rank-bottleneck throwaway pilot (§3.1.7) | One discarded pilot, trained with the query projection actually fixed at `rank=36` (not full-rank), before any of the six real-seed runs. Same §3.2 terminal-checkpoint criterion — competence judged only at the terminal checkpoint / complete history, a passing streak mid-run is not the verdict (pilots 0/1 looked identical to this at step 100). **Also a budget check**: its `S*`, computed under §3.1's identical stable-competence definition, is compared against 25,600 — if it exceeds that, the training budget re-derives as `4 × max(25600, rank-pilot S*)` before any real seed launches | FIXED (procedure); **result PENDING** — currently running, in progress at last check |
...
| `raw/*.jsonl` volume, Phase 3-4 projected | Phase 1's figure is Phase 1 only. Phase 3: 2 probe positions × ~8-point layer/step sweep × ~3-4 cross-condition combos ≈ **~48×**. Phase 4: constrained-vs-free (2×) × ~4-config recursion-budget sweep ≈ **~8×**. Rough total ≈ `8.1MB × 57 ≈ 462MB/run`, **`~2.8GB for all six`** — an estimate, not a measurement (Phase 3/4 pass counts aren't preregistered yet), but sizes the manifest scheme correctly now rather than at Phase 3 launch | FIXED (as an order-of-magnitude projection, explicitly not a measurement) |
