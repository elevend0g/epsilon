...
| Margin AUROC criterion | `≥ 0.70`, predicting top-1 retrieval correctness | **FIXED, measured, FAIL (undefined) on all six checkpoints** — `model/phase2_margin.py`, `runs/phase2_margin_result.json`, measured 2026-08-23. AUROC requires both a positive (correct) and negative (incorrect) class; **zero incorrect retrieval steps occurred in 294,912 pooled teacher-forced steps across all six checkpoints** (49,152 each, all three `L`, arm A). AUROC is not computable, not merely low. Not reinterpreted as a pass — reported as measured |
| Top-1 retrieval accuracy criterion | within `[0.60, 0.98]` on arm A | **FIXED, measured, FAIL on all six checkpoints** — top-1 accuracy `= 1.0000` (uniformly, every checkpoint, every `L`, pooled and per-`L`), outside the upper bound. §4's "all must pass before any coverage or recursion claim" is read literally: **Phase 2 is not fully cleared. Phase 3/4 do not proceed on this criterion's account until it is explicitly resolved**, not silently passed through. See `RESULTS.md` for the full writeup and why this is a coherent (not obviously erroneous) consequence of task structure, not treated as license to waive the criterion |
