...
| First-hop reliability floor | **Resolved, was training-instability, not architecture.** Under the unstable constant-LR recipe, pilot 2 showed `m=0` on arm A at rates up to 6.6% (`L=2`) and 7.2% (`L=3`) — the training-instability artifact the row used to flag as unconfirmed. Re-measured 2026-08-22 on the stable cosine-schedule pilots: `m=0` at `L=2`/`L=3` is 0.6%/0.5% (pilot 0), 0%/0.1% (pilot 1), 0.35%/0.65% (pilot 2) — all comfortably under the original flat 5% target. The floor-relative §5.3 restatement stands as the more defensible general principle regardless (would have caught this proactively rather than describing it after training stabilized), but the specific number it responded to no longer applies | FIXED, resolved — `RESULTS.md` finding 4 |
