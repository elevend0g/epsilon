...
| Total real-seed commitment (§2.6) | 2 supervision regimes × 3 seeds = **6 runs**, budget is per-run: `6 × 102,400 = 614,400` optimizer steps total, before any Phase 2-4 instrumentation | FIXED — confirm affordable before launching any of the six, not after |
...
| Rank-bottleneck throwaway pilot (§3.1.7) | One discarded pilot, trained with the query projection actually fixed at `rank=36` (not full-rank), before any of the six real-seed runs. Same §3.2 terminal-checkpoint criterion. Validates the bottleneck itself, which no pilot to date has ever exercised — §3.1.5 mandates full-rank pilots by construction | FIXED (procedure); **result PENDING** |
