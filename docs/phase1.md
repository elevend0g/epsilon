...
**The arm-A form of this criterion is unsatisfiable by construction, not merely failed (measured 2026-08-23, `RESULTS.md` finding 13).** Top-1 accuracy on arm A came back exactly `1.0000` across 294,912 pooled teacher-forced retrieval steps, all six real seeds, zero exceptions — outside `[0.60, 0.98]`, and leaving margin AUROC undefined for lack of a negative class. This is a structural consequence, not an artifact of one unlucky measurement: there is no error-correction across hops, so any checkpoint that clears §3.2's much harder whole-walk `≥95%` gate is close to structurally required to retrieve perfectly on the same in-distribution arm-A data this criterion measures. No remeasurement at a different point in training escapes this — a checkpoint that hasn't yet reached near-perfect per-step accuracy is a checkpoint that hasn't passed §3.2 either, and Phase 2 only ever runs on checkpoints that have.

**Superseded by a B-arm criterion, fixed here before it is measured.** Retrieval is perfect exactly where a queried key exists; the only way margin AUROC can be well-posed at all is against a population that includes queries for keys that genuinely do not exist. Arms B1, B2, and C already contain exactly this structure without any new generator work: for any B1/B2/C item, teacher-forcing the walk through its own valid prefix (`§2.5`'s `ρ` valid hops) produces `ρ` genuine present-key queries (the key exists, retrieval should find it), and the very next query — the one at which the walk actually breaks — is a genuine absent-key query (no cache entry has that key at all; arm C's break is its first query, at `ρ=0`). Arm A/D's every query is present-key by construction.

**Label, precisely:** for each teacher-forced query, label `1` (present-key) if the queried key exists as some cache entry's key in this item's memory, `0` (absent-key) otherwise. Positives are pooled from arm A/D's full walk and B1/B2's own valid prefixes; negatives are pooled from B1/B2/C's single break-point query per item, one per item by construction (the walk breaks exactly once).

**Fixed criterion, before measurement: margin AUROC (present-key vs. absent-key, as labeled above) ≥ 0.70.** Same threshold as the original — 0.70 was already justified as "meaningfully better than chance discrimination" and there is no principled reason to pick a different number now that the arm-A form's failure is understood. The accuracy-window half of the original criterion has no natural analogue here (there is no "correct index" to score accuracy against when the key is absent) and is not reinvented as a substitute; AUROC alone carries the criterion. This is precisely the signal the §2.2 gate's margin input is supposed to carry — if it comes back weak, that is an architecture-level finding to have before Phase 3, not after.

**A related, explicitly preregistered prediction for Phase 3, from the same day's counterfactual findings:** §4.2's clamped arm showed Regime 2's margin collapsing roughly seven-fold further than Regime 1's when an integration is withheld (`RESULTS.md` finding 12) — the closest signal this project has produced toward Q2, arriving from an arm-A intervention rather than the coverage probe itself. **Full specification — the magnitude threshold and how to interpret each outcome — is fixed in §5.4, not restated here**, to keep one authoritative version rather than two that could drift.
...
**Every P2 probe, in either regime, gets a second and third regress-out control: whether the pos2 retrieval landed on an END-flagged cache entry, and the integration count leading into pos2.** Originally scoped to Regime 2 only, from finding 11's END-guess-rate asymmetry under a foreign cache (Regime 2 ~7% vs Regime 1 ~3%) — **that scoping was wrong, caught on the same day it was written.** The actual mechanism is structural, not regime-specific: arm A's pos2 query is *by construction* the item's own correct END-retrieval, so `is_end_2` is true for nearly all of arm A's items regardless of which regime trained the checkpoint; B2's failure mode (a corrupted value, not a missing key) has no structural reason to land on an END entry at all. Nothing about that argument depends on Regime 2's training exposure to dead ends — it holds for any checkpoint that solves arm A, Regime 1 included. `AMENDMENTS.md` #2 records the correction. Integration count is added as a third control for the same reason P1's entire raw signal turned out to be a count shortcut (§2.5's `ρ` asymmetry between arm A and B2) — that asymmetry is a property of the arms, not of which probe position is being read, so it applies at pos2 too. End-flag identity is a binary covariate (`pos2` retrieval is/isn't END-flagged); integration count is the cumulative gate-fire count through pos2. All three are residualized separately (diagnostic) and jointly (the actual question: does anything survive once every known structural confound is removed at once) — same OLS/bootstrap-CI treatment as the original margin control.
...
**Regime 2 prediction, magnitude fixed before Phase 3 runs (from §4.2's clamped-arm finding, `RESULTS.md` finding 12 — a roughly sevenfold margin-collapse asymmetry between regimes under intervention).** Ordering alone ("Regime 2 higher than Regime 1") is not a real test — it would be satisfied by a difference as small as `0.71` vs `0.70`. Confirmation requires **both**: Regime 2's AUROC `≥ 0.80`, **and** the gap `(Regime 2 AUROC − Regime 1 AUROC) ≥ 0.15`. `0.80` sits a full tier above the `0.70` "meaningfully better than chance" bar already used twice in this document (§4.3's original and superseding criteria) and clear of Regime 1's own `0.65` falsification line — a sevenfold raw asymmetry predicts a qualitatively strong result, not a bare pass. The `0.15` gap requirement is sized to match: substantial relative to AUROC's `0.5–1.0` informative range, closing the loophole a trivial ordering would leave open.

**What a Regime-1-null-plus-Regime-2-strong result means, decided now because it is the single most likely outcome given everything measured so far, and the framing must not be improvised after the number arrives.** Regime 2 was trained directly on B1/B2/C → ABSTAIN; a strong coverage-probe result there is close to confirming the model learned what it was explicitly taught, which is not the same claim Q2 exists to test. **Regime 1 is the number Q2 actually turns on** — §5.4's falsification bound is written against Regime 1 specifically because that arm tests whether dead-end representation emerges in a network *never* trained to report it (§2.4). If Regime 2 confirms (clears `0.80`, `≥0.15` above Regime 1) while Regime 1 falls at or below `0.65`: **report the Regime 1 null as the headline finding, not as a footnote to Regime 2's confirmation.** A summary that leads with "Regime 2 succeeded" in this scenario misstates what was actually shown. Q2 fails on its own preregistered terms regardless of how strong Regime 2's number is — a strong Regime 2 does not partially rescue a falsified Regime 1, and must not be written up as though it does.
