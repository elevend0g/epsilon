...
**A related, explicitly preregistered prediction for Phase 3 (§5.4), from the same day's counterfactual findings:** §4.2's clamped arm showed Regime 2's margin collapsing roughly seven-fold further than Regime 1's when an integration is withheld (`RESULTS.md` finding 12) — the closest signal this project has produced toward Q2, arriving from an arm-A intervention rather than the coverage probe itself. **Prediction, stated before Phase 3 runs: Regime 2 should show higher cross-condition transfer separation than Regime 1 on the §5.4 coverage probe** (arm A vs. B2 at position P2). §5.4's Regime-1 falsification threshold (`AUROC < 0.65`) is unchanged; this is an additional, directional prediction about the Regime 1-vs-2 comparison specifically, not a new numeric gate. If Regime 2 does *not* separate more than Regime 1 despite this arm-A asymmetry, that is a disconfirmation worth its own discussion, not a result to explain away.
